...
class TreeNode:
    def __init__(self, data, name):
        ...
        self.path_length = 0 # the total path length up to this node
...
def a_star(root): # A* Algorithm Function, takes the root (initial state) as an argument
    print("A* SOLUTION")
    if root.data == 0: # Checks if initial state is goal state
    ...
    while frontier: # the loop to find the goal state
        ...
        if not inExplored and current.leftChild != 0: # if not explored AND exists, inject to queue
            current.leftChild.path_length = current.path_length + 1
            current.leftChild.total = current.leftChild.data + current.leftChild.path_length
        ...
        if not inExplored and current.rightChild != 0: # if not explored AND exists, inject to queue
            current.rightChild.path_length = current.path_length + 1
            current.rightChild.total = current.rightChild.data + current.rightChild.path_length
            frontier.inject(current.rightChild)

    print("No solution") # print statement for no solution
    return -1 # return case for no solution

a_star(root) # calls the A* algorithm with _0 as the initial state
...
'''
My A* implementation takes the same path as I took manually when I simulated the problem. Since the fork occurs at
the very beginning, the algorithm alternates between the same paths for a bit since they have the same Manhattan costs
until the path that goes down from the initial state starts to double back. At that point, the algorithm commits to the
path that starts left of the initial state the rest of the way.
'''